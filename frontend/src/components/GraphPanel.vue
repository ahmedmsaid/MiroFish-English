    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content still processing. Manually refresh graph later.</span>
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // From simulating to non-simulating, show finished hint
...
const renderGraph = () => {
  ...
  // Process edge data, calculate number and index of edges between the same pair of nodes
  const edgePairCount = {}
  ...
  // Record which edge of each node pair is currently being processed
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Calculate curvature: multiple edges are spaced out, single edge is straight
    let curvature = 0
    if (totalCount > 1) {
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance based on number of edges between node pair
      // Base distance 150, increase by 50 for each extra edge
    ...
    // Add centripetal force to gather independent node groups
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Depart from right side, wrap around
      const x1 = sx + 8  // Start offset
      const y1 = sy - 4
      const x2 = sx + 8  // End offset
      const y2 = sy + 4
      // Use elliptical arc to draw self-loop (sweep-flag=1 clockwise)
    ...
    // Vertical offset, calculated proportionally to distance
    // More edges mean higher offset ratio
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, increase by 5% for each extra edge
  ...
  // Calculate link midpoint (for label positioning)
  const getLinkMidpoint = (d) => {
    ...
    // Quadratic Bezier curve midpoint formula B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position, don't restart simulation yet (distinguish between click and drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // Restart simulation only after real drag detected
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only stop simulation gradually if actually dragged
        if (d._isDragging) {
