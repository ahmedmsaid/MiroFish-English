...
const props = defineProps({
  ...
  minutesPerRound: {
    type: Number,
    default: 30 // Default 30 mins per round
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Check if simulation is complete (via runner_status or platform status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Additional check: if backend hasn't updated runner_status yet, but platforms report complete
      // Judge via twitter_completed and reddit_completed status
...
const checkPlatformsCompleted = (data) => {
  ...
  // Judge if platform is enabled by actions_count (if count > 0 or running was true)
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Incremental add new actions (deduplicated)
      ...
      // Don't auto-scroll, allow users to freely browse timeline
      // New actions appended at the bottom
    }
  } catch (err) {
