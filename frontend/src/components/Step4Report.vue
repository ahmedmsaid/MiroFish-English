                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'section_start'">
                    <div class="section-tag">
                      <span class="section-index">Section {{ log.section_index }}</span>
                      <span class="tag-title">{{ log.section_title }}</span>
                    </div>
                  </template>
                  
                  <!-- Section Content Generated (Content generation complete, but the entire section might not be) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const relSection = text.match(/### (?:Relation Chains|\[Relation Chains\])\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const reasonMatch = text.match(/### Reason for Selection\n([\s\S]*?)(?=\n---\n|\n### Interview Transcript)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^- Select\s*([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        // Split questions by numerical index
      ...
      // Extract answers - Split between Twitter and Reddit
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy to default if not placeholder
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" (English format) or "Question X:" (Chinese format, new backend format)
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // If split successfully and index is valid
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // If split failed: first question returns full answer, others return empty
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual platform toggle button (only displayed if there are real dual platform answers)
                  ...
                  // Expand/Collapse Button (placeholder text does not show)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading secondary title (## xxx), as section title is already displayed externally
  ...
  // Process line breaks - empty lines become paragraph breaks, single line breaks become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled uniformly in nextTick after the loop
          }
...
const extractFinalContent = (response) => {
  ...
  // If the content is long and contains markdown format, try to return it after removing the thinking process
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Dark gray, non-reactive to state */
  font-weight: 500;
}
