<template>
  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- Card Header: simulation_id and Functionality Availability -->
...
const containerStyle = computed(() => {
  ...
  // Expanded: dynamically calculate height based on number of cards
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate current row card count to ensure centering
    ...
    // Expand downwards, increase spacing with title
    ...
    const x = offset * 35
    // Adjust starting position, close to title but maintaining proper spacing
...
// Navigate to Graph Construction (Project)
const goToProject = () => {
...
// Navigate to Env Setup (Simulation)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (record latest target state even during animation)
        ...
        // If animating, only record state, handle after animation finishes
        ...
        // Debounce state switch to prevent flicker
        // Shorter delay (50ms) when expanding, longer (200ms) when collapsing for stability
        const delay = shouldExpand ? 50 : 200
        
        expandDebounceTimer = setTimeout(() => {
          // Double check if animating
          if (isAnimating) return
          
          // Double check if pending state still needs execution
          ...
          // Release lock after animation finishes, check for pending transitions
          setTimeout(() => {
            isAnimating = false
            
            // Check for new pending state after animation
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay execution to avoid rapid switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, contract bottom upward: scroll more to trigger expand
...
onMounted(async () => {
  ...
  // Init observer after DOM rendering
  setTimeout(() => {
    initObserver()
  }, 100)
})

// If using keep-alive, reload data on activation
onActivated(() => {
  loadHistory()
})
...
/* Simplified display when no project */
...
/* Use CSS background pattern to create a fixed-pitch square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Position from top-left, only extend at bottom when height changes, no effect on existing grid positions */
...
.cards-container {
  ...
  /* min-height is dynamically calculated by JS, adapt according to the number of cards */
}
...
/* ===== History Playback Details Popup Styles ===== */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
