...
const isSimulating = computed(() => {
  // If we are in Step 3 and the status is processing, it means simulation is running
...
const handleNextStep = () => {
  // Step3Simulation component will handle report generation and navigation directly
  ...
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        addLog(`Failed to get time configuration, using default value: ${minutesPerRound.value} min/round`)
      }
...
const startGraphRefresh = () => {
  ...
  addLog('Real-time graph refresh enabled (30s)')
...
onMounted(() => {
  ...
  // Log maxRounds configuration (value already obtained from query parameters during initialization)
  if (maxRounds.value) {
