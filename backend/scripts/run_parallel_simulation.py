...
- Do not close the environment immediately after completing the simulation, enters command waiting mode
...
# This is to fix issues where third-party libraries (like OASIS) read files without specified encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Force default encoding (affecting the default encoding of the open() function)
    # Note: This needs to be set when Python starts; setting it during runtime may not take effect
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        This can fix issues where third-party libraries (like OASIS) read files without specified encoding
        """
        # Only set default encoding for text mode (non-binary) and where encoding is not specified
...
# Load .env file from the project root directory (contains LLM_API_KEY, etc.)
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai's max_tokens warnings (we deliberately do not set max_tokens, letting the model decide)"""
...
# Add filter immediately when module loads, ensuring it takes effect before camel code execution
...
class ParallelIPCHandler:
    ...
    def __init__(
        ...
        Args:
            ...
            twitter_agent_graph: Twitter agent graph
            reddit_env: Reddit environment
            reddit_agent_graph: Reddit agent graph
        """
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        if not env or not agent_graph:
            return {"platform": platform, "error": f"{platform} platform unavailble"}
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        if success_count > 0:
            ...
            print(f"  Interview completed: agent_id={agent_id}, count of successful platforms={success_count}/{len(platforms_to_interview)}")
            return True
        else:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (can be overridden by each interview item)
                - "twitter": Interview Twitter platform only
                - "reddit": Interview Reddit platform only
                - None/not specified: Interview both platforms simultaneously for each Agent
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
        ...
        if twitter_interviews and self.twitter_env:
            try:
                ...
                print(f"  Batch Twitter Interview failed: {e}")
        ...
        if reddit_interviews and self.reddit_env:
            try:
                ...
                print(f"  Batch Reddit Interview failed: {e}")
...
# Non-core action types that need to be filtered out (these actions are of lower analytical value)
FILTERED_ACTIONS = {'refresh', 'sign_up'}

# Action type mapping table (names in database -> standard names)
ACTION_TYPE_MAP = {
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows displaying the real entity name in actions.jsonl instead of a code name like "Agent_0"
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Maximum rowid value from last read (using rowid instead of created_at because created_at formats differ across platforms)
        agent_names: agent_id -> agent_name mapping
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: List of actions, each element containing agent_id, agent_name, action_type, action_args (including context information)
    ...
    try:
        ...
        # This avoids problems with created_at format differences (Twitter uses integers, Reddit uses datetime strings)
        cursor.execute("""
...
def _enrich_action_context(
    ...
    try:
        ...
        # Repost: supplement original post content and author
            ...
            if new_post_id:
                # The original_post_id of the reposted post points to the original post
                cursor.execute("""
        ...
        # Failure to supplement context does not affect the main process
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Support dual LLM configuration for speedup during parallel simulation:
    ...
    If boost LLM is configured, different platforms can use different API service providers during parallel simulation to improve concurrency.
    ...
    # If there's no model name in .env, use config as fallback
    if not llm_model:
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("Missing API Key configuration, please set LLM_API_KEY in the .env file in the project root directory")
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Maximum simulation rounds (optional, used to truncate simulations that are too long)
...
async def run_reddit_simulation(
    ...
    # Reddit uses accelerated LLM configuration (if available, otherwise falls back to general configuration)
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum simulation rounds (optional, used to truncate simulations that are too long)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
    ...
    # Create shutdown event at the start of the main function to ensure the entire program can respond to exit signals
    ...
    log_manager.info(f"Simulation loop completed! Total time taken: {total_elapsed:.1f}s")
...
def setup_signal_handlers(loop=None):
    """
    Set signal handlers to ensure correct exit upon receiving SIGTERM/SIGINT
