...
Zep's node/edge list interface uses UUID cursor for pagination. 
This module encapsulates automatic page-turning logic (including single-page retry) 
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single page request, retry with exponential backoff on failure. Only retry network/IO transient errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch graph nodes by page, returning at most max_items (default 2000). Each page request includes retries."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all graph edges by page, returning the complete list. Each page request includes retries."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
