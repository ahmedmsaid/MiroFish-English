...
# Adding this prefix prevents the Agent from calling tools and uses text response instead
INTERVIEW_PROMPT_PREFIX = "Combined with your persona, all past memories and actions, reply to me directly with text without calling any tools:"


def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview question, add prefix to avoid Agent calling tools
    ...
    Request (JSON):
        {
            ...
            "graph_id": "mirofish_xxxx",    // Optional, defaults to project's graph_id
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    """
    Check if simulation preparation is completed
    ...
    Note: Run scripts (run_*.py) stay in backend/scripts/ directory, no longer copied to simulation directory
    ...
    # Necessary files list (excluding scripts, scripts are in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # Detailed logs
        logger.debug(f"Detected simulation preparation status: {simulation_id}, status={status}, config_generated={config_generated}")
        
        # If config_generated=True and files exist, consider preparation complete
        # All following statuses mean preparation work is complete:
        # - ready: Preparation complete, ready to run
        # - preparing: If config_generated=True it means completed
        # - running: Already running, means preparation was completed long ago
        # - completed: Run complete, means preparation was completed long ago
...
def prepare_simulation():
    ...
    Features:
    - Auto-detect completed preparation work, avoiding repeated generation
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started|Already completed preparation",
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Already completed preparation, no need to regenerate",
        ...
        # This allows the frontend to get the expected total Agents immediately after calling prepare
        try:
            ...
            # Failure doesn't affect downstream, background task will retry
        ...
        # Update simulation status (containing pre-fetched entity count)
...
def get_prepare_status():
    ...
    2. Check if completed preparation work already exists via simulation_id
    
    Request (JSON):
        {
            "task_id": "task_xxxx",          // Optional, task_id returned by prepare
            "simulation_id": "sim_xxxx"      // Optional, simulation ID (for checking already completed prep)
        }
    ...
    try:
        ...
        # If simulation_id provided, check if already completed preparation first
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
        ...
        if not task:
            # Task does not exist, but if simulation_id provided, check if already completed preparation
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation_history():
    ...
    Used for historical project display on the homepage, returns a simulation list 
    containing rich information like project name, description, etc.
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get running status (read actual rounds set by user from run_state.json)
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    - Directly reads file without going through SimulationManager
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read profiles file (might be being written): {e}")
                profiles = []
        
        # Check if currently generating (detect via state.json)
        is_generating = False
        total_expected = None
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns extra metadata (e.g., file modification time, generation status)
    - Returns partial information even if config is not fully generated
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read config file (might be being written): {e}")
                config = None
...
def download_simulation_script(script_name: str):
    """
    Download simulation script file (general script, located in backend/scripts/)
...
# ============== Profile Generation Interfaces (Standalone Use) ==============
...
def start_simulation():
    ...
    About force parameter:
        - When enabled, if simulation is running or completed, it will stop and clean up running logs first
        ...
        - Suitable for scenarios requiring re-running simulation

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posts, comments, likes, etc.) in simulation will be updated to Zep graph in real-time
    ...
    try:
            ...
            if is_prepared:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            try:
                                ...
                                logger.warning(f"Warning appearing while stopping simulation: {str(e)}")
                        else:
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is running, please call /stop interface to stop, or use force=true to force restart"
                            }), 400
                ...
                if force:
                    ...
                    if not cleanup_result.get("success"):
                        logger.warning(f"Warning appearing while cleaning logs: {cleanup_result.get('errors')}")
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Enabling graph memory update requires a valid graph_id, ensure project has built graph"
                }), 400
...
def get_run_status_detail(simulation_id: str):
    """
    Get detailed simulation running status (containing all actions)
    ...
    try:
        ...
        # recent_actions only shows content of two platforms for current latest round
...
def get_simulation_timeline(simulation_id: str):
    ...
    Returns summarized info for each round
    """
    try:
        ...
        if not os.path.exists(db_path):
            return jsonify({
                ...
                "data": {
                    ...
                    "count": 0,
                    "posts": [],
                    "message": "Database does not exist, simulation might not have run yet"
                }
            })
...
def interview_agent():
    ...
    Note: This feature requires simulation environment to be in running state (enters wait-for-command mode after completing simulation loop)
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been closed. Please ensure the simulation has completed and entered wait-for-command mode."
            }), 400
...
def interview_agents_batch():
    ...
    Note: This feature requires simulation environment to be in running state
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What's your opinion on B?"  // If platform not specified, use default value
                }
            ],
            "platform": "reddit",              // Optional, default platform (overridden by item platform)
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required, simulation ID
            "prompt": "What's your overall opinion on this?",  // Required, interview question (same question for all Agents)
...
def get_interview_history():
    ...
    Request (JSON):
        {
            ...
            "agent_id": 0,                 // Optional, get summary for specific Agent
    ...
    try:
        ...
        simulation_id = data.get('simulation_id')
        platform = data.get('platform')  # Returns history of both platforms if not specified
        agent_id = data.get('agent_id')
...
def close_simulation_env():
    ...
    while this interface will let the simulation close the environment gracefully and exit.
