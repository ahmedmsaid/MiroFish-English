...
ONTOLOGY_SYSTEM_PROMPT = """You are a professional knowledge graph ontology design expert. Your task is to analyze given text content and simulation requirements, designing entity types and relationship types suitable for **social media public opinion simulation**.
...
- Entities will influence each other, retweet, comment, respond
- We need to simulate the reactions of all parties and information propagation paths in public opinion events

Therefore, **entities must be subjects that truly exist in reality and can speak and interact on social media**:
...
- Specific group representatives (such as alumni associations, fan clubs, rights protection groups, etc.)
...
**Hierarchy requirements (must contain both specific types and fallback types)**:

Your 10 entity types must contain the following levels:

A. **Fallback types (must be included, placed in the last 2 of the list)**:
   - `Person`: Fallback type for any individual person. Categorized here when a person does not belong to other more specific person types.
   - `Organization`: Fallback type for any organization. Categorized here when an organization does not belong to other more specific organization types.
...
- Various people appear in the text, such as "primary and secondary school teachers", "passerby A", "a certain netizen"
...
**Personal (Fallback)**:
- Person: Any individual (use when not belonging to the above specific types)
...
class OntologyGenerator:
    ...
    def generate(
        ...
        result = self.llm_client.chat_json(
            messages=messages,
            temperature=0.3,
            max_tokens=32000  # Reasoning-type models (e.g., GLM-4.7) require a large amount of tokens for reasoning, the JSON output itself also needs enough space
        )
    ...
    def _build_user_message(
        ...
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            ...
            combined_text += f"\n\n...(Original text {original_length} characters, truncated to the first {self.MAX_TEXT_LENGTH_FOR_LLM} characters for ontology analysis)..."
...
5. Attribute names cannot use reserved words like name, uuid, group_id, replace them with full_name, org_name, etc.
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if fallbacks_to_add:
            ...
            if current_count + needed_slots > MAX_ENTITY_TYPES:
                ...
                # Remove from the end (preserve more important specific types at the beginning)
