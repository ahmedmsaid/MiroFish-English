...
class AgentActivity:
    ...
    def _describe_like_post(self) -> str:
        """Like post - contains post content and author info"""
    ...
    def _describe_dislike_post(self) -> str:
        """Dislike post - contains post content and author info"""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_repost(self) -> str:
        """Repost - contains original post content and author info"""
    ...
    def _describe_quote_post(self) -> str:
        """Quote post - contains original post content, author info, and quote comment"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
    ...
    def _describe_follow(self) -> str:
        """Follow user - contains name of followed user"""
    ...
    def _describe_create_comment(self) -> str:
        """Create comment - contains comment content and info of post being commented on"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_generic(self) -> str:
        # For unknown action type, generate generic description
...
class ZepGraphMemoryUpdater:
    ...
    Monitors simulation action log files and updates Zep graph with new agent activities in real-time.
    Grouped by platform, batch sends to Zep after accumulating BATCH_SIZE activities.
    
    All meaningful behaviors are updated to Zep, and action_args contain full context:
    - Post content for Likes/Dislikes
    - Original post content for Reposts/Quotes
    - Usernames for Follows/Mutes
    - Comment content for Likes/Dislikes
    """
    
    # Batch size (send after accumulating this many activities per platform)
    BATCH_SIZE = 5
    ...
    # Send interval (seconds) to avoid rapid requests
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Activity buffers grouped by platform (batch send when BATCH_SIZE reached)
        ...
        self._total_sent = 0        # Successful batch sends to Zep
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args contain full context info (e.g. post content, username, etc.).
    ...
    def _worker_loop(self):
        """Background worker loop - periodically send activities to Zep per platform"""
            ...
            try:
                ...
                try:
                    ...
                    with self._buffer_lock:
                        ...
                        # Check if threshold reached for platform
                        if len(self._platform_buffers[platform]) >= self.BATCH_SIZE:
                            batch = self._platform_buffers[platform][:self.BATCH_SIZE]
                            self._platform_buffers[platform] = self._platform_buffers[platform][self.BATCH_SIZE:]
                            # Release lock before sending
                            self._send_batch_activities(batch, platform)
                            # Interval between sends to avoid rapid requests
    ...
    def _flush_remaining(self):
        ...
        # First process activities remaining in queue, add to buffers
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "batches_sent": self._total_sent,            # Successful batch sends
            "items_sent": self._total_items_sent,        # Successful activity items sent
