...
Converts entities from Zep graph to the Agent Profile format required by the OASIS simulation platform

Optimization improvements:
1. Call Zep retrieval function to enrich node information
...
class OasisProfileGenerator:
    ...
    Converts entities from Zep graph to the Agent Profile required for OASIS simulation
    ...
    # Group/institution type entities (need to generate representative personas)
    GROUP_ENTITY_TYPES = [
    ...
    def _generate_username(self, name: str) -> str:
        ...
        # Add random suffix to avoid repetition
        suffix = random.randint(100, 999)
        return f"{username}_{suffix}"
    
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Retrieve rich information related to the entity using Zep graph hybrid search
        
        Zep does not have a built-in hybrid search interface, so it needs to search edges and nodes separately and then merge results.
        ...
        comprehensive_query = f"All information, activities, events, relationships, and backgrounds about {entity_name}"
        ...
        try:
            # Execute edge and node search in parallel
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            try:
                ...
                # 5. If still failing, attempt more aggressive fix
                try:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in social media user profile generation. Generate detailed, authentic personas for public opinion simulation, restoring the existing reality to the greatest extent possible. Must return valid JSON format, all string values must not contain unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, restoring the existing reality to the greatest extent possible.
...
2. persona: Detailed persona description (2000 words of plain text), should include:
   ...
   - Background (important experiences, association with the event, social relationships)
   - Personality traits (MBTI type, core personality, emotional expression style)
   - Social media behavior (posting frequency, content preference, interaction style, linguistic features)
   - Stance and views (attitude towards the topic, content that might provoke/move them)
   - Unique traits (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of the persona, introducing the association with the event, as well as existing actions and reactions)
...
Important:
- All field values must be strings or numbers, no newlines allowed
- persona must be a coherent text description
- Use English (except for specific proper nouns if necessary)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account persona for the institutional entity, matching its official identity and function.
...
2. persona: Detailed account setting description (2000 words of plain text), should include:
   - Basic info (official name, nature, background, functions)
   - Account positioning (type, target audience, core functions)
   - Speaking style (linguistic features, common expressions, taboo topics)
   - Content features (content types, posting frequency, active hours)
   - Stance (official position on core topics, handling controversies)
   - Special notes (representative group persona, operating habits)
   - Institutional memory (association with the event, existing actions and reactions)
3. age: Fixed as 30 (virtual age for institutional accounts)
4. gender: Fixed as "other" (representing non-personal identity)
...
Important:
- All values must be strings or numbers, no null values allowed
- persona must be a coherent text description, no newlines
- Use English
- age must be 30, gender must be "other"
- Speaking style must match its identity positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        """Generate basic persona based on rules"""
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Write to file in real-time (even fallback persona)
                    save_profiles_realtime()
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        ...
        # Output only to console (avoid repetition, logger no longer outputs full content)
        print(output)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profile as CSV format (matches OASIS official requirements)
        
        OASIS Twitter required CSV fields:
        - user_id: User ID (starts from 0 according to CSV order)
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Uses same format as to_reddit_format(), ensuring OASIS can read correctly.
        Must include user_id field, which is key for matching in OASIS agent_graph.get_agent()!
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                ...
                # OASIS required fields - ensure default values
