...
# Mark whether cleanup function is registered
...
class SimulationRunState:
    ...
    # Platform-independent rounds and simulated hours (for parallel display)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Platform-independent rounds and hours
    ...
    def to_detail_dict(self) -> Dict[str, Any]:
        """Contains detailed information for recent actions"""
        result = self.to_dict()
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"Simulation configuration not found, please call /prepare interface first")
        ...
        try:
            ...
            # Add max-rounds command line argument if specified
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])
            
            # Create main log file to avoid process blocking when stdout/stderr buffer is full
            ...
            # Keep file handles to close later
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        # New log structure: separate platform action logs
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Update individual platform rounds and hours
                                    if platform == "twitter":
                                    ...
                                    # Total rounds takes the max of two platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Total time takes the max of two platforms
    ...
    def _read_actions_from_file(
        ...
        Args:
            file_path: Path to action log file
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Skip non-action records (events like simulation_start, round_start, round_end, etc.)
                    if "event_type" in data:
                        continue
                    
                    # Skip records without agent_id (not an Agent action)
                    if "agent_id" not in data:
                        continue
                    
                    # Get platform: priority to platform in record, otherwise use default platform
    ...
    def get_all_actions(
        ...
        """
        Get full action history for all platforms (no pagination limit)
        ...
        if not actions:
            ...
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Old format should have platform field in file
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def get_timeline(
        ...
        Returns:
            Round-by-round summary information
        """
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        - twitter_simulation.db (platform database)
        - reddit_simulation.db (platform database)
    ...
    def cleanup_all_simulations(cls):
        ...
        Called when server closes to ensure all child processes are terminated
        """
        ...
        # Check if anything needs cleaning (to avoid printing useless logs for empty processes)
    ...
    def register_cleanup(cls):
        ...
        Called when Flask app starts, ensuring all simulation processes are cleaned up when server closes
        """
        ...
        # In Flask debug mode, only register cleanup in reloader child process (actual app process)
        # WERKZEUG_RUN_MAIN=true indicates reloader child process
        # If not in debug mode, there is no such environment variable, register always
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # In debug mode, only register in reloader child process; otherwise always register
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark as registered to prevent re-registration in child process
            return
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            # Only print logs if there are processes needing cleanup
        ...
        try:
            ...
            logger.warning("Failed to register signal handlers (not in main thread), using atexit only")
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specific platform (optional)
                ...
                - None: Interview both platforms in a dual-platform simulation, return integrated result
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, will be overridden by platform in each interview item)
                ...
                - None: Interview both platforms for each Agent in a dual-platform simulation
            timeout: Timeout period (seconds)
    ...
    def interview_all_agents(
        ...
        Use the same question to interview all Agents in simulation
    ...
    def close_simulation_env(
        ...
        Sends a close environment command to simulation, making it exit gracefully from waiting mode
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment close command sent (waiting for response timeout, environment may be closing)"
            }
    ...
    def get_interview_history(
        ...
        Args:
            ...
            limit: Return limit per platform
