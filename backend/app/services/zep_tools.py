...
Encapsulates tools for graph search, node reading, edge queries, etc., for use by Report Agent
...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            f"## Deep Analysis for Future Prediction",
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        if self.active_facts:
            text_parts.append(f"\n### [Currently Valid Facts] (Original simulation text)")
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # Truncate content that's too long (truncate at period instead of hard truncation)
...
class ZepToolsService:
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        ...
        # LLM client for InsightForge to generate sub-questions
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges from the graph (paginated, includes temporal information)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Fetches all edges in the graph and filters for those related to the specified node
        ...
        try:
            # Fetch all edges in graph and filter
            all_edges = self.get_all_edges(graph_id)
    ...
    def get_entities_by_type(
        ...
        """
        logger.info(f"Fetching entities for type {entity_type}...")
    ...
    def insight_forge(
        ...
        Most powerful hybrid retrieval function, automatically decomposes problem and performs multi-dimensional retrieval:
        1. Uses LLM to decompose problem into multiple sub-queries
        2. Perform semantic search for each sub-query
        3. Extract relevant entities and fetch their detailed information
        ...
        # Perform search on original query as well
        ...
        # Step 4: Build all relationship chains (not limited)
        relationship_chains = []
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are a professional problem analysis expert. Your task is to decompose a complex problem into multiple sub-queries that can be independently observed in the simulated world.
...
Return sub-query list in JSON format."""
    ...
    def panorama_search(
        ...
        Get full panorama view, including all relevant content and historical/expired information:
        ...
        This tool is suitable for scenarios needing an overall view of events or tracking evolution processes.
        ...
        # Fetch all edges (including temporal info)
        ...
        for edge in all_edges:
            ...
            if is_historical:
                # Historical/expired fact, add time label
    ...
    def interview_agents(
        ...
        Calls real OASIS interview API to interview Agents currently running in simulation:
        ...
        4. Calls /api/simulation/interview/batch interface for real interviews (simultaneous multi-platform)
        5. Integrates all interview results to generate an interview report
        
        [IMPORTANT] This function requires simulation environment to be running (OASIS environment not closed)
        ...
        # Consolidate questions into an interview prompt
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "4. Answer questions one by one according to question number, each answer starts with 'Question X:' (X is question number)\n"
            "5. Use empty lines to separate answers for each question\n"
            "6. Answers should have substantial content, with at least 2-3 sentences per question.\n\n"
        )
        optimized_prompt = f"{INTERVIEW_PROMPT_PREFIX}{combined_prompt}"
        
        # Step 4: Call real interview API (no platform specified, multi-platform simultaneous interview by default)
        try:
            # Build batch interview list (no platform specified, multi-platform interview)
            interviews_request = []
            ...
            logger.info(f"Calling batch interview API (Multi-platform): {len(interviews_request)} Agents")
            
            # Call SimulationRunner batch interview method (platform=None for multi-platform interview)
            api_result = SimulationRunner.interview_agents_batch(
                simulation_id=simulation_id,
                interviews=interviews_request,
                platform=None,  # No platform specified, multi-platform interview
                timeout=180.0   # Multi-platform needs longer timeout
            )
            ...
            # Multi-platform return format: {"twitter_0": {...}, "reddit_0": {...}, "twitter_1": {...}, ...}
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Get interview results for the Agent from the two platforms
                ...
                # Clean up possible tool call JSON packaging
                twitter_response = self._clean_tool_call_response(twitter_response)
                reddit_response = self._clean_tool_call_response(reddit_response)

                # Always output dual platform indicator
                ...
                # Extract key quotes (from responses of both platforms)
                import re
                ...
                # Strategy 2 (Supplementary): Long text within correctly paired quotes (like 「」 or "")
                if not key_quotes:
        ...
        logger.info(f"InterviewAgents complete: Interviewed {result.interviewed_count} Agents (Dual platforms)")
        return result
    
    @staticmethod
    def _clean_tool_call_response(response: str) -> str:
        """Clean up JSON tool call packaging in Agent reply, extract actual content"""
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable interviewee from the list of simulated Agents based on the interview requirements.
...
Please select up to {max_agents} Agents best suited for interview and explain the reasons."""
    ...
    def _generate_interview_questions(
        ...
        system_prompt = """You are a professional reporter/interviewer. Generate 3-5 deep interview questions based on the requirements.

Question requirements:
1. Open-ended questions, encouraging detailed answers
2. Different answers possible for different roles
3. Cover multiple dimensions such as facts, opinions, feelings, etc.
4. Natural language, like a real interview
5. Each question controlled within 50 words, concise and clear
...
Please generate 3-5 interview questions."""
    ...
    def _generate_interview_summary(
        ...
        # Collect all interview contents
        interview_texts = []
...
2. Point out consensus and divergence in viewpoints
3. Highlight valuable quotes
4. Objective and neutral, do not favor any party
5. Stay within 1000 words
...
- Use English quotes "" when quoting interviewees' original words
...
Please generate interview summary."""
        ...
        try:
            ...
            return f"Interviewed {len(interviews)} interviewees in total, including: " + ", ".join([i.agent_name for i in interviews])
