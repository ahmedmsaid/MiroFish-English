...
Adopts a step-by-step generation strategy to avoid failure caused by once-off generation of long content:
...
# China daily routine time configuration (Beijing Time)
...
class TimeSimulationConfig:
    """Time simulation configuration (based on Chinese people's daily habits)"""
    # Total simulation duration (simulation hours)
    total_simulation_hours: int = 72  # Default simulation of 72 hours (3 days)
    
    # Time represented per round (simulation minutes) - default 60 minutes (1 hour), speed up time flow
    minutes_per_round: int = 60
    
    # Range of Agent activations per hour
    agents_per_hour_min: int = 5
    agents_per_hour_max: int = 20
    
    # Peak hours (19-22:00 in the evening, most active time for Chinese people)
    peak_hours: List[int] = field(default_factory=lambda: [19, 20, 21, 22])
    peak_activity_multiplier: float = 1.5
    
    # Off-peak hours (0-5:00 in the early morning, almost no activity)
...
class PlatformConfig:
    ...
    # Viral propagation threshold (propagation triggered after how many interactions)
    viral_threshold: int = 10
    
    # Echo chamber effect strength (degree of clustering of similar views)
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    temperature=0.7 - (attempt * 0.1)  # Decrease temperature with each retry
                    # No max_tokens set, let LLM run freely
                )
...
### Basic principles (for reference only, need flexible adjustment according to specific events and participating groups):
- The user group is Chinese, adhering to Beijing Time daily routine
- 0-5:00 AM almost no activity (activity coefficient 0.05)
- 6-8:00 AM gradually becoming active (activity coefficient 0.4)
- 9-18:00 work hours medium activity (activity coefficient 0.7)
- 19-22:00 evening peak period (activity coefficient 1.5)
- Activity decreases after 23:00 (activity coefficient 0.5)
- General pattern: early morning low activity, morning gradual increase, work hours medium, evening peak
- **Important**: The above example values are for reference only, you need to adjust specific periods according to event nature and group characteristics.
  - E.g., student peak might be 21-23:00; media is active all day; official institutions only during work hours.
  - E.g., sudden hot topics might cause discussion late at night, off_peak_hours can be shortened accordingly.
...
Field description:
- total_simulation_hours (int): Total simulation hours, 24-168 hours, short for sudden events, long for continuous topics
...
- peak_hours (int array): Peak period, adjust according to participant groups
- off_peak_hours (int array): Low peak period, usually late night/early morning
...
- reasoning (string): Brief explanation of the configuration"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. Time configuration should match Chinese lifestyle habits."
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time configuration result and ensure agents_per_hour values do not exceed total agent count"""
        # Get raw values
        ...
        return TimeSimulationConfig(
            ...
            off_peak_activity_multiplier=0.05,  # Almost no activity at 0-5am
    ...
    def _generate_event_config(
        ...
        # List representative entity examples for each type
        type_examples = {}
        for e in entities:
...
- Design initial post contents, **each post must specify poster_type (poster type)**

**Important**: poster_type must be selected from the "Available Entity Types" above so that initial posts can be assigned to appropriate Agent publishers.
E.g., official statements should be published by Official/University types, news by MediaOutlet, student views by Student.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        # Type alias table (handling different formats LLM might output)
        type_aliases = {
        ...
        # Record used index for each type to avoid reusing the same agent
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                ...
                if agent_configs:
                    # Sort by influence, select the highest one
...
- **Time matches Chinese people's daily routine**: Almost no activity from 0-5:00 AM, most active from 19-22:00 PM
...
- **Individuals** (Student/Person/Alumni): High activity (0.6-0.9), active mainly in the evening (18-23), fast response (1-15 minutes), low influence (0.8-1.2)
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<active hour list, considering Chinese routine>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON, configurations must match Chinese people's habit routine."
        ...
        for i, entity in enumerate(entities):
            ...
            # If LLM failed to generate, use rule-based generation
            if not cfg:
