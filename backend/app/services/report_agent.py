...
3. Each section adopts ReACT multi-round thinking and reflection mode
4. Supports dialogue with users, autonomously calling retrieval tools in the dialogue
"""
...
class ReportLogger:
    ...
    Generates agent_log.jsonl file in the report folder to record detailed actions for each step.
    ...
    def log_section_content(
        ...
        """Record section content generation complete (only records content, does not represent the entire section completion)"""
        self.log(
...
class ReportConsoleLogger:
    ...
    def _setup_file_handler(self):
        """Set up the file handler to write logs to a file at the same time"""
        import logging
...
This tool is used to obtain a complete full view of the simulation results, especially suitable for understanding the event evolution process. It will:
...
[Deep Interview - Real Agent Interview (Dual Platforms)]
Call the interview API of the OASIS simulation environment to conduct real interviews with running simulation Agents!
This is not LLM simulation, but calling real interview interfaces to get the original answers of simulation Agents.
...
PLAN_SYSTEM_PROMPT = """\
You are a "Future Prediction Report" writing expert, with a "God's View" of the simulated world—you can insight into every Agent's behavior, speech, and interaction in the simulation.

[Core Philosophy]
We have constructed a simulated world and injected specific "Simulation Requirements" as variables. The evolution results of the simulated world are predictions of what might happen in the future. What you are observing is not "experimental data," but a "preview of the future."
...
[Report Positioning]
- ✅ This is a simulation-based future prediction report, revealing "what the future will be like if this happens"
- ✅ Focus on predicted results: event direction, group reactions, emergent phenomena, potential risks
- ✅ Agent words and deeds in the simulated world are predictions of future human behavior
- ❌ Not an analysis of the current state of the real world
- ❌ Not a vague public opinion summary
...
The simulated world is a preview of the future. We have injected specific conditions (Simulation Requirement) into the simulated world,
...
1. [Must call tools to observe the simulated world]
   - You are observing the preview of the future from a "God's View"
   - All content must come from events that occurred in the simulated world and Agent words and deeds
   - Prohibit using your own knowledge to write report content
...
2. [Must quote original Agent words and deeds]
   - Agent words and deeds are predictions of future human behavior
   - Use quoting format to display these predictions in the report, for example:
     > "A certain type of group will express: original content..."
   - These quotes are core evidence for simulation predictions

3. [Language Consistency]
   - The report should be written entirely in English.
   - When you quote content returned by tools, ensure it is seamlessly integrated into the English report.
   - If the content is in another language, translate it to fluent English before writing it into the report.
...
- ❌ Prohibit using any Markdown headers (#, ##, ###, ####, etc.) within the section
- ❌ Prohibit adding a section main header at the beginning of the content
- ✅ Section headers are automatically added by the system, you only need to write pure body content
- ✅ Use **bold**, paragraph separation, quotes, lists to organize content, but do not use headers
...
**First Launch Incitement Phase**

Twitter, as the first scene of public opinion, assumed the core function of information first launch:

> "Twitter contributed 68% of the first launch volume..."

**Emotion Amplification Phase**

The TikTok platform further amplified the event influence:

- Visual impact is strong
- Emotional resonance degree is high
```

[Incorrect Example]
```
## Executive Summary          ← Incorrect! Do not add any headers
### 1. First Launch Phase     ← Incorrect! Do not use ### for sub-sections
#### 1.1 Detailed Analysis   ← Incorrect! Do not use #### for sub-division
...
- panorama_search: Wide-angle panorama search, understand event full view, timeline and evolution process
- quick_search: Quickly verify a specific information point
- interview_agents: Interview simulation Agents, get first-person views and real reactions of different roles

═══════════════════════════════════════════════════════════════
[Workflow]
═══════════════════════════════════════════════════════════════

Each time you reply, you can only do one of the following two things (cannot do both at the same time):
...
When you have obtained sufficient information through tools, start with "Final Answer:" to output the section content.

⚠️ Strictly Prohibited:
- Prohibit including both tool call and Final Answer in one reply
...
2. Cite originals extensively to showcase simulation effects
3. Use Markdown format (but prohibit using headers):
   ...
   - ❌ Prohibit using any header syntax like #, ##, ###, ####
4. [Quote Format Specification - Must be independent paragraph]
   ...
   > "The school's coping mode appears rigid and slow in the rapidly changing social media environment."

   This evaluation reflected the general dissatisfaction of the public.
   ```

   ❌ Incorrect format:
   ```
   The school's response was considered to lack substantive content. > "The school's coping mode..." This evaluation reflected...
   ```
5. Maintain logical consistency with other sections
6. [Avoid Duplication] Read the completed section content below carefully, do not repeat the same information
7. [Again Emphasize] Do not add any headers! Use **bold** instead of sub-section headers"""
...
- ✅ Section headers are automatically added by the system
- ✅ Write the body directly, use **bold** instead of sub-section headers

Please start:
1. First think (Thought) what information this section needs
2. Then call tool (Action) to obtain simulation data
3. After collecting enough information, output Final Answer (pure body, without any headers)"""
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Note] You only called tools {tool_calls_count} times, at least {min_tool_calls} times are required."
...
3. Only call retrieval tools for more data when the report content is insufficient to answer.
...
- Give conclusion first, then explain the reason."""
...
class ReportAgent:
    ...
    2. Generation Stage: Generate content section by section, each section can call tools multiple times to get information
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # Deep interview - Call real OASIS interview API to get simulation Agent responses (dual platforms)
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only try if format 1 doesn't match, to avoid mis-matching JSON in the body
        ...
        # The response may contain thought text + bare JSON, try to extract the last JSON object
    ...
    def plan_outline(
        ...
        Analyze simulation requirements using LLM, plan report directory structure
    ...
    def _generate_section_react(
        ...
        Args:
            ...
            previous_sections: Content of previous sections (for consistency)
        ...
        conflict_retries = 0  # Continuous conflict count for tool call and Final Answer appearing simultaneously
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Break loop and enter forced completion if last iteration returns None
                break
            ...
            if has_tool_calls and has_final_answer:
                ...
                logger.warning(
                    ...
                    f"LLM output both tool call and Final Answer ({conflict_retries} conflicts)"
                )
                ...
                if conflict_retries <= 2:
                    ...
                    logger.warning(
                        f"Section {section.title}: {conflict_retries} continuous conflicts, "
        ...
        if response is None:
            logger.error(f"Section {section.title} LLM returned None during forced completion, using default error hint")
    ...
    def generate_report(
        ...
        """
        Generate full report (output section by section in real-time)
        
        Saves each section to the folder immediately after completion, no need to wait for the entire report to complete.
        File Structure:
        reports/{report_id}/
        ...
        # If no report_id passed, generate automatically
        if not report_id:
    ...
    def chat(
        ...
        The Agent can independently call retrieval tools to answer questions in the dialogue
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Starting line to read (for incremental retrieval, 0 means from start)
    ...
    def get_console_log_stream(cls, report_id: str) -> List[str]:
        """
        Get full console log (retrieve all at once)
    ...
    def get_agent_log_stream(cls, report_id: str) -> List[Dict[str, Any]]:
        """
        Get full Agent log (retrieve all at once)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        1. Remove Markdown heading lines duplicating the section title at the start
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # As section titles are added by the system, there should be no headings in the content
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        # Post-processing: Clean heading issues throughout the report
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate headings (headings with the same content within 5 consecutive lines)
                is_duplicate = False
                ...
                # Skip divider line immediately following heading
                i += 1
                continue
    ...
    def delete_report(cls, report_id: str) -> bool:
        ...
        if os.path.exists(folder_path) and os.path.isdir(folder_path):
            try:
                ...
                logger.error(f"Failed to delete report folder: {e}")
                return False
